...
export default function Discussion({ courseId, subSectionId }) {
  ...
  return (
    <div className="mt-6 flex flex-col gap-4">
      <h3 className="text-lg font-semibold text-richblack-5">Q&amp;A / Discussion</h3>

      {/* Post form */}
      <form onSubmit={handlePost} className="bg-richblack-800 rounded-xl p-4 flex flex-col gap-3 border border-richblack-600">
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="Your question title..."
          className="bg-richblack-700 text-richblack-5 text-sm rounded-md px-3 py-2 outline-none border border-richblack-600 focus:border-yellow-400"
        />
        <textarea
          value={content}
          ...
          rows={3}
          className="bg-richblack-700 text-richblack-5 text-sm rounded-md px-3 py-2 outline-none border border-richblack-600 focus:border-yellow-400 resize-none"
        />
        <button
          type="submit"
          disabled={loading}
      ...
      {discussions.length === 0 ? (
        ...
        discussions.map((d) => (
          <div key={d._id} className={`bg-richblack-800 rounded-xl p-4 border ${d.isResolved ? "border-green-600/40" : "border-richblack-600"}`}>
            ...
            {openReply === d._id && (
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-richblack-700 text-richblack-5 text-sm rounded-md px-3 py-1.5 outline-none border border-richblack-600 focus:border-yellow-400"
                />
                <button
